...
class HomeCubit extends Cubit<HomeState> {
  ...
  final GetNearestCarsUseCase getNearestCarsUseCase;

  HomeCubit({
    required this.getNearestCarsUseCase,
  ...
  Future<void> fetchBrands({bool isRefresh = false}) async {
    ...
    final result =
        await getBrandsUseCase(PageCurrentCarsParams(page: state.currentPage));
  ...
  Future<void> fetchNearestCars(int locationId) async {
    print('🔄 Fetching nearest cars for location: $locationId');
    emit(state.copyWith(status: AppStatus.loading));

    final result = await getNearestCarsUseCase(
      GetNearestCarsParams(locationId: locationId, page: 1),
    );
    ...
    result.fold(
      ...
      (carsResponse) {
        print('✅ SUCCESS: ${carsResponse.cars.length} nearest cars loaded');

        for (var car in carsResponse.cars) {
          print('  🚗 ${car.name}');
          print('     Location: ${car.location?.name}');
          print('     Main Image URL: ${car.mainImageUrl}');
        }

        emit(state.copyWith(
          status: AppStatus.success,
          nearestCars: carsResponse.cars,
          hasReachedMax:
              carsResponse.meta.currentPage >= carsResponse.meta.lastPage,
        ));
      },
  ...
  Future<void> loadMoreNearestCars(int locationId) async {
    if (state.hasReachedMax || state.status == AppStatus.loading) return;

    final currentPage = (state.nearestCars!.length ~/ 5) + 1;
    print('📄 Loading nearest cars page $currentPage...');

    final result = await getNearestCarsUseCase(
      GetNearestCarsParams(locationId: locationId, page: currentPage),
    );
    ...
    result.fold(
      ...
      (carsResponse) {
        ...
        final updatedCars = [...state.nearestCars!, ...carsResponse.cars];

        emit(state.copyWith(
          status: AppStatus.success,
          nearestCars: updatedCars,
          hasReachedMax:
              carsResponse.meta.currentPage >= carsResponse.meta.lastPage,
        ));
      },
